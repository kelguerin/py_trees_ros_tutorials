...
.. code-block:: bash
   ...
   $ py-trees-render -b hugr.six_context_switching.tutorial_create_root

.. graphviz:: dot/tutorial-six-context-switching.dot
   :align: center

.. literalinclude:: ../hugr/six_context_switching.py
   :language: python
   :linenos:
   :lines: 132-232
...
The :class:`hugr.behaviours.ScanContext` is the
context switching behaviour constructed for this tutorial.

* :meth:`~hugr.behaviours.ScanContext.initialise()`: trigger a sequence service calls to cache and set the /safety_sensors/enabled parameter to True
* :meth:`~hugr.behaviours.ScanContext.update()`: complete the chain of service calls & maintain the context
* :meth:`~hugr.behaviours.ScanContext.terminate()`: reset the parameter to the cached value
...
On entry into the parallel, the :class:`~hugr.behaviours.ScanContext`
...
the :class:`~hugr.behaviours.ScanContext` will terminate,
...
.. code-block:: bash
    ...
    # Launch the tutorial
    $ ros2 launch hugr tutorial_six_context_switching_launch.py
